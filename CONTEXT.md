...
## Project Overview
This project is a Google Chrome Extension designed to provide quick access to board game information. It bridges the gap between reading about a game on any website and finding its specific stats.

## Architecture
The extension follows the Manifest V3 architecture:
- **Service Worker (`background.js`)**: Handles the business logic. It listens for context menu clicks, performs asynchronous requests to the XML API, parses the response (handling XML parsing manually/regex since DOMParser is limited in some SW environments, though handled carefully here), and sends structured data back to the content script.
